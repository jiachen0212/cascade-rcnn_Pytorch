...
class _FPN(nn.Module):
    ...
    def _PyramidRoI_Feat(self, feat_maps, rois, im_info):
        ...
        if cfg.POOLING_MODE == 'crop':
            ...
            roi_pool_feats = []
            box_to_levels = []
            # P2~P6层的proposals一起统计起来,因为是共享一个DetHead的～
...
# ----------------not be used ---------------
        ...
        if self.training:
            ...
            # loss (l1-smooth) for bounding box regression
        ...
        # 2nd-----------------------------
        # decode
        # bbox_decode(rois, bbox_pred, 把 1nd 的 bbox拿；来用 ...
...
# ----------------not be used ---------------
        if self.training and not self.class_agnostic:
            ...
            bbox_pred_select = torch.gather(bbox_pred_view, 1,
                                            rois_label.long().view(rois_label.size(0), 1, 1).expand(rois_label.size(0),                                                                                                1, 4))
            bbox_pred = bbox_pred_select.squeeze(1)
# ----------------not be used ---------------
        ...
        # 3rd---------------
        # decode   bbox_decode(rois, bbox_pred_2nd,
